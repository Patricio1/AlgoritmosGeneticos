...
public class Controlador implements Initializable{
    ...
    private void accionStart(ActionEvent event) {
		...
		textArea.setText(textArea.getText()+Funcion.toString(individuos)+"\n\n");
		...
		for (int i=0; i<numGeneraciones; i++){
			...
			for(int j=0; j<numIndividuos; j++){
				double valor = Funcion.funcion((int) valorIndividuos[j]);
			...
			generarMutaciones();
			textArea.setText(textArea.getText()+Funcion.toString(mutaciones)+"\n");
			mutar();
			...
			textArea.setText(textArea.getText()+"\nEl valor de su función de calidad es: "+Funcion.funcion((int) valorIndividuos[getMejorIndividuo()-1])+"\n");
		
			tfGeneracion.setText(""+(i+1));
			tfMejorIndividuo.setText(""+valorIndividuos[getMejorIndividuo()-1]);
			tfFuncionCalidad.setText(""+Funcion.funcion((int) valorIndividuos[getMejorIndividuo()-1]));
	...
	public static void setValorIndividuo (){
		for (int i=0; i<numIndividuos; i++){
			valorIndividuos [i] = Funcion.binario_a_entero(individuos[i]);
		}
	}
	...
	public static void calcularTramosSelección (){
		...
		for (int i=0; i<numIndividuos; i++){
			sum += Funcion.funcion((int) valorIndividuos[i]);
		}
		
		double contador = 0;
		// Normalizamos los valores para hacer la seleccion
		for (int i=0; i<numIndividuos; i++){
			double val = Funcion.funcion((int) valorIndividuos[i])/(double) sum;
	...
	public static int getMejorIndividuo (){
		...
		for (int i=0; i<numIndividuos; i++){
			double valorFuncion = Funcion.funcion((int) valorIndividuos[i]);
			if (valorFuncion > fun){
				fun = valorFuncion;
				individuo = i;
			}
	...
	private void inicializarGrafica() {
		...
		for (double i = 0.0; i < getRango(numCromosomas); i = i + 0.5) {
			series.getData().add(new XYChart.Data<Double, Double>(i, Funcion.funcion(i)));
		}
